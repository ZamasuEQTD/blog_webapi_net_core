...
namespace WebApp
{
    static public class MediaInjection
    {
        public static void AddMediaDependecies(this IServiceCollection services)
        {
            ...
            services.AddScoped<IVistaPreviaHelper, VistaPreviaHelper>(vp => new(Path.Join(services.BuildServiceProvider().GetService<IWebHostEnvironment>()!.ContentRootPath, "Media", "VistasPrevias")));
            services.AddScoped<IMiniaturaHelper, MiniaturaHelper>(mp => new(services.BuildServiceProvider().GetService<IArchivosHelper>()!, services.BuildServiceProvider().GetService<IImagesHelper>()!,
                Path.Join(services.BuildServiceProvider().GetService<IWebHostEnvironment>()!.ContentRootPath, "Media", "Thumbnails")
            ));
